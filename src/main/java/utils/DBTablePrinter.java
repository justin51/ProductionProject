 ...
 * (<a target="_blank" href="http://stackoverflow.com">stackoverflow.com</a>) was the primary source
 * of inspiration and help to put this code together. Especially the questions and answers of the
 * following people were very useful:</p>
 *
 * <p>Question:
 ...
 * from database into a table</a><br> People: Simon Cottrill, Tony Toews, Costis Aivali, Riggy,
 * corsiKa</p>
 *
 * <p>Question:
...
public class DBTablePrinter {
  ...
  /**
   * Column type category for <code>REAL</code>, <code>DOUBLE</code>, and <code>DECIMAL</code>
   * columns.
   */
  ...
  /**
   * Column type category for types for which the type name will be printed instead of the content,
   * like <code>BLOB</code>,
  ...
  private static class Column {
    ...
    /**
     * Generic SQL type of the column as defined in <a target="_blank"
     * href="http://docs.oracle.com/javase/8/docs/api/java/sql/Types.html"> java.sql.Types
     * </a>.
     */
    private int type;

    /**
     * Generic SQL type name of the column as defined in <a target="_blank"
     * href="http://docs.oracle.com/javase/8/docs/api/java/sql/Types.html"> java.sql.Types
     * </a>.
     */
    ...
    /**
     * Width of the column that will be adjusted according to column label and values to be
     * printed.
     */
    private int width = 0;
    ...
    /**
     * Flag for text justification using <code>String.format</code>. Empty string (<code>""</code>)
     * to justify right, dash (<code>-</code>) to justify left.
     *
     * @see #justifyLeft()
     */
    ...
    /**
     * Column type category. The columns will be categorised according to their column types and
     * specific needs to print them correctly.
     */
    private int typeCategory = 0;

    /**
     * Constructs a new <code>Column</code> with a column label, generic SQL type and type name (as
     * defined in <a target="_blank" href="http://docs.oracle.com/javase/8/docs/api/java/sql/Types.html">
     * java.sql.Types
     * </a>)
     *
    ...
    /**
     * Adds a <code>String</code> representation (<code>value</code>) of a value to this column
     * object's {@link #values} list. These values will come from each row of a <a target="_blank"
    ...
    /**
     * Returns the column value at row index <code>i</code>. Note that the index starts at 0 so
     * that
     * <code>getValue(0)</code> will get the value for this column from the first row of a <a
     * target="_blank" href="http://docs.oracle.com/javase/8/docs/api/java/sql/ResultSet.html">
     * ResultSet</a>.
     *
    ...
    /**
     * Sets {@link #justifyFlag} to <code>"-"</code> so that the column value will be left justified
     * when printed with
  ...
  /**
   * Overloaded method that prints rows from table <code>tableName</code> to standard out using the
   * given database connection
   * <code>conn</code>. Total number of rows will be limited to
   * {@link #DEFAULT_MAX_ROWS} and {@link #DEFAULT_MAX_TEXT_COL_WIDTH} will be used to limit the
   * width of text columns (like a <code>VARCHAR</code> column).
   *
   ...
   * {@link #DEFAULT_MAX_TEXT_COL_WIDTH} will be used to limit the width of text columns (like a
  ...
  public static void printTable(Connection conn, String tableName, int maxRows,
    ...
    if (conn == null) {
      System.err
          .println("utils.DBTablePrinter Error: No connection to database (Connection is null)!");
      return;
    }
    if (tableName == null) {
      System.err.println("utils.DBTablePrinter Error: No table name (tableName is null)!");
      return;
    }
    if (tableName.length() == 0) {
      System.err.println("utils.DBTablePrinter Error: Empty table name!");
      return;
    }
    if (maxRows < 1) {
      System.err.println("utils.DBTablePrinter Info: Invalid max. rows number. Using default!");
    ...
    try {
      if (conn.isClosed()) {
        System.err.println("utils.DBTablePrinter Error: Connection is closed!");
        return;
      }
      ...
      System.err.println("SQL exception in utils.DBTablePrinter. Message:");
   ...
   * ResultSet</a> to standard out using {@link #DEFAULT_MAX_TEXT_COL_WIDTH} to limit the width of
   * text columns.
   *
  ...
  /**
   * Overloaded method to print rows of a <a target="_blank" href="http://docs.oracle.com/javase/8/docs/api/java/sql/ResultSet.html">
   * ResultSet</a> to standard out using <code>maxStringColWidth</code> to limit the width of text
   * columns.
   *
   * @param rs                The <code>ResultSet</code> to print
   * @param maxStringColWidth Max. width of text columns
  ...
  public static void printResultSet(ResultSet rs, int maxStringColWidth) {
    try {
      if (rs == null) {
        System.err.println("utils.DBTablePrinter Error: Result set is null!");
        return;
      }
      if (rs.isClosed()) {
        System.err.println("utils.DBTablePrinter Error: Result Set is closed!");
        return;
      }
      if (maxStringColWidth < 1) {
        System.err
            .println(
                "utils.DBTablePrinter Info: Invalid max. varchar column width. Using default!");
  ...
  /**
   * Takes a generic SQL type and returns the category this type belongs to. Types are categorized
   * according to print formatting needs:
   * <p>
   * Integers should not be truncated so column widths should be adjusted without a column width
   * limit. Text columns should be left justified and can be truncated to a max. column width
   * etc...</p>
   * <p>
